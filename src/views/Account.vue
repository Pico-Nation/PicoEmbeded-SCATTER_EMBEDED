			...
			async moderateResource(resource){
				new Promise(async resolve => {
					...
					if(['CPU', 'NET'].includes(name))
						PopupService.push(Popup.picoModerateCpuNet(this.account, returnResult));

					if(name === 'RAM')
						PopupService.push(Popup.picoModerateRam(this.account, returnResult));
			...
			async commitAction(action){
				...
				switch(action.type){
					...
					case 'change_permissions': return PopupService.push(Popup.verifyPassword(verified => {
						...
						PopupService.push(Popup.picoChangePermissions(this.account, async permissions => {
							await plugin.changePermissions(this.account, permissions);
						}));
					}));
					case 'proxy_vote': return PopupService.push(Popup.picoProxyVotes(this.account, () => {}));
					case 'create_account': return PopupService.push(Popup.picoCreateAccount(this.account, () => {}));
					default: return '';
				}
