			...
			<section class="keypair" :key="keypair.id" v-for="keypair in filteredKeypairs" :class="{'new':isNew(keypair), 'selectable':keypairsOnly}" @click="$emit('keypair', keypair)">
				<section class="keypair-info">
					...
					<section class="actions" v-if="!asSelector">
						...
						<section class="action-menu" :class="{'hidden':actionsMenu !== keypair.id}">
							...
							<figure class="item" v-if="keypair.enabledKey().blockchain === Blockchains.PICOIO" @click="filterLinkedAccounts(keypair)">
								<i class="icon-user"></i> Unlink Unselected Accounts
							</figure>

							<figure class="item" v-if="holdingCtrl && keypair.enabledKey().blockchain === Blockchains.PICOIO" @click="linkAccount(keypair)">
				...
				<section v-if="!keypairsOnly">
					...
					<section class="no-accounts" v-else>
						...
						<Button v-if="canCreateAccounts(keypair)" :text="$t('keysAndAccountsList.noAccounts.createAccountButton')" @click.native="createPicoAccount(keypair)" />
					</section>
				</section>
...
<script>
	...
	export default {
		...
		methods:{
			...
			linkAccount(keypair){
				PopupService.push(Popup.picoLinkAccount(keypair))
			},
			...
			createPicoAccount(keypair){
				PopupService.push(Popup.picoCreateAccount(keypair, done => {
