				...
				<section>
					...
					<section v-for="item in changeableKeys" class="key-entry">
						...
						<section class="split-inputs">
							...
							<Input style="margin-bottom:0; flex:1;" placeholder="PICO..." :text="keys[item.ref]" v-on:changed="x => keys[item.ref] = x" />
						</section>
					</section>
...
<script>
	...
	export default {
		...
		computed:{
			...
			validKeypairs(){
				return this.keypairs.filter(x => x.blockchains.includes(Blockchains.PICOIO))
			},
			canUseExchange(){
			...
			network(){
				const endorsed = PluginRepository.plugin(Blockchains.PICOIO).getEndorsedNetwork();
			...
			totalPrice(){
				return (parseFloat(this.ramPrice ? this.ramPrice : 0) + parseFloat(this.picoToUse ? this.picoToUse : 0)).toFixed(this.decimals);
			},
			systemSymbol(){
		...
		methods:{
			...
			finishedAccountCreation(tx){
				setTimeout(async () => {
					PopupService.push(Popup.transactionSuccess(Blockchains.PICOIO, tx));
					...
					if(owner && (!active || active.id !== owner.id)) await AccountService.importAllAccounts(owner, false, [Blockchains.PICOIO], [this.account.network()]);
					if(active) await AccountService.importAllAccounts(active, false, [Blockchains.PICOIO], [this.account.network()]);
			...
			async createAccount(){
				...
				const plugin = PluginRepository.plugin(Blockchains.PICOIO);
				this.picoToUse = parseFloat(this.picoToUse).toFixed(this.decimals);
			...
			async checkAccountName(){
				...
				accountTimeout = setTimeout(async () => {
					...
					if(!PluginRepository.plugin(Blockchains.PICOIO).isValidRecipient(this.accountName)){
