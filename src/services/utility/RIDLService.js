...
export default class RIDLService {
	...
	static async checkApp(app){
		if(!StoreService.get().state.scatter.settings.firewall) return;
		await this.init();
	...
	static async createAccount(){
		...
		if(!keypair){
			...
			keypair.blockchains = [Blockchains.PICOIO];
		...
		const publicKey = keypair.publicKeys.find(x => x.blockchain === Blockchains.PICOIO).key;
	...
	static async getCycleData(){
		...
		return fetch(`${network.fullhost()}/v1/chain/get_table_rows`, {
			...
			if(!x.rows.length) return {cycle, tokens:'0.0000 PICO'};
			return x.rows[0];
		}).catch(error => {
			return {cycle, tokens:'0.0000 PICO'};
		})
	}

	static async donateToScatter(account, quantity){

		quantity = parseFloat(quantity.split(' ')[0]).toFixed(4) + ' PICO';

		return new Promise(async (resolve, reject) => {
			const plugin = PluginRepository.plugin(Blockchains.PICOIO);
			const pico = plugin.getSignablePicojs(account, reject);
			...
			await pico.transact({
				...
				.then(trx => {
					PopupService.push(Popup.transactionSuccess(Blockchains.PICOIO, trx.transaction_id));
					resolve(trx);
				})
				.catch(res => {
					console.error(error);
	...
	static async claimRidlTokens(accounts){
		return new Promise(async (resolve, reject) => {
			const plugin = PluginRepository.plugin(Blockchains.PICOIO);
			const pico = plugin.getSignablePicojs(accounts, reject);
			...
			await pico.transact({ actions }, { blocksBehind: 3, expireSeconds: 30, })
				.then(res => {
					PopupService.push(Popup.transactionSuccess(Blockchains.PICOIO, res.transaction_id));
					resolve(res);
